...
//
//#define SA0 P3_2        // Slave Address Bit (D/C# for SA0)
...
unsigned char length = 8;     // Send Command
  ...
  while(length--)
  {
    if(mcmd & 0x80)
    {
      digitalWrite(SDA_PIN, 1); // SDA=1;
    }
    else
    {
      digitalWrite(SDA_PIN, 0); // SDA=0;
    }
    uDelay(3);
    digitalWrite(SCL_PIN, 1); // SCL=1;
    uDelay(3);
    digitalWrite(SCL_PIN, 0); // SCL=0;
    uDelay(3);
    mcmd = mcmd << 1;
  }
}
...
//  Patterns
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//unsigned char code UniV[][48]={     // UniV OLED
...
unsigned char i;
  ...
  digitalWrite(RES_PIN, 0); // RES=0;
  for(i=0;i<200;i++)
  {
    uDelay(200);
  }
  digitalWrite(RES_PIN, 1); // RES=1;
...
////            // WiseChip
////unsigned char code Tel[]={11,24,24,22,13,19,23,13,21,24,23,17,22,24,0};
////            // +886-37-587168
////unsigned char i;
////
////  P1=0xFF;
...
////            // Downward - Top Area
////    Continuous_Scroll(0x00,0x01,0x00,0x03,XLevel,XLevel+Max_Column-1,0x00,0x20,0x01,0x00,0x02);
////            // Up & Rightward - Top Area
////    Continuous_Scroll(0x01,0x01,0x00,0x03,XLevel,XLevel+Max_Column-1,0x00,0x20,0x1F,0x00,0x02);
////            // Down & Leftward - Top Area
////    Continuous_Scroll(0x01,0x01,0x04,0x07,XLevel,XLevel+Max_Column-1,0x00,0x20,0x01,0x00,0x02);
////            // Upward - Top Area
////            // Leftward - Bottom Area
////    Continuous_Scroll(0x00,0x01,0x04,0x07,XLevel,XLevel+Max_Column-1,0x00,0x20,0x1F,0x00,0x02);
////            // Downward - Top Area
